...
public:
    vector<vector<int>> floodFill(vector<vector<int> >& image, int sr, int sc, int color) {
        ...
        setColor(image, image[sr][sc], sr, sc, color);
        return image;
    }

    void setColor(vector<vector<int> >& image, int from, int sr, int sc, int color) {
        int r = image.size();
        int c = image[0].size();
        ...
        int cur =  image[sr][sc];
        image[sr][sc] = color;
        setColor(image, cur,  sr - 1, sc, color);
        setColor(image, cur,  sr + 1, sc, color);
        setColor(image, cur,  sr, sc - 1, color);
        setColor(image, cur,  sr, sc + 1, color);
        return ;
    }
};
