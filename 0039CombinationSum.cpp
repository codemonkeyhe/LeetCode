...
public:
    vector<vector<int> > combinationSum(vector<int>& candidates, int target) {
        ...
        int sum = 0;
        int idx = 0;
        bt(candidates, target, idx, sum, path, res);
        return  res;
    }

    void bt(vector<int>& nums, int target, int idx, int& sum, vector<int>& path, vector<vector<int> >& res) {
        if (sum == target) {
            res.push_back(path);
            return ;
        }
